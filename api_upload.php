...
/**
 * api_upload.php
 * Standart Fotoğraf Yükleme | ORİJİNAL KORUMA + CACHE (ÖNİZLEME) OLUŞTURMA | TRAFİK KOTASI KONTROLÜ
 */
...
function createWebCache($source, $destination, $scaleRatio = 0.20, $quality = 80) {
    ...
    // ORANTISAL PIXEL KÜÇÜLTME (Normal fotoğraflar için %80 küçültme, yani orijinalin %20'si kalır)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['photos'])) {
    ...
    // Klasör oluşturma (Normal fotoğraflar için 'albumler' klasörü)
    $target_dir = __DIR__ . "/uploads/albumler/{$firma_id}/{$musteri_id}/";
    ...
    for ($i = 0; $i < $num_files; $i++) {
        ...
        // 1. Orijinal dosyayı olduğu gibi kaydet
        if (move_uploaded_file($tmp, $dest_path_original)) {
            // 2. Web önizlemesi için küçültülmüş kopyayı (cache) oluştur
            // Seçim ekranında fotoğraflar mermi gibi aksın diye %80 oranında küçültüyoruz (ScaleRatio = 0.20)
            createWebCache($dest_path_original, $dest_path_cache, 0.20, 80);
